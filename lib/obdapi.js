...
class ObdApi {
    constructor({ url = DEFAULT_URL, loginPhrase = '', mnemonic = '', data = shapes_1.defaultDataShape, saveData = () => null, listeners = {}, selectedNetwork = 'bitcoin', onOpen = () => null, onClose = () => null, onError = () => null, }) {
        this.defaultUrl = url;
    ...
    connect({ url, data, saveData, loginPhrase, mnemonic, listeners, selectedNetwork, onMessage, onChannelCloseAttempt, onChannelClose, onOpen, onError, onClose, onAddHTLC, onForwardR, onSignR, onCloseHTLC, }) {
        return new Promise((resolve) => {
            ...
            this.data = data ? data : shapes_1.defaultDataShape;
            if (saveData)
                this.saveData = saveData;
            if (loginPhrase)
                this.loginPhrase = loginPhrase;
