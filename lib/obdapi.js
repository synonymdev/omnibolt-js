...
class ObdApi {
    ...
    getDataFromServer(jsonData) {
        ...
        if (jsonData.status == false) {
            if (jsonData.type == this.messageType.MsgType_Core_Omni_Getbalance_2112) {
                try {
                    if (callback != null)
                        return callback(this.failure("Omni Error"));
                }
                catch {
            ...
            try {
                if (callback != null)
                    return callback(this.failure(jsonData));
            }
            catch {
                return this.failure(jsonData);
            }
        ...
        if (callback != null)
            callback(this.success(resultData));
    ...
    bitcoinFundingSigned(recipient_node_peer_id, recipient_user_peer_id, info, callback) {
        ...
        if (this.isNotString(info.signed_miner_redeem_transaction_hex)) {
            return callback(this.failure("empty signed_miner_redeem_transaction_hex"));
        }
        let msg = new pojo_1.Message();
    ...
    addHTLC(recipient_node_peer_id, recipient_user_peer_id, info, callback) {
        ...
        if (this.isNotString(info.curr_htlc_temp_address_for_ht1a_pub_key)) {
            return callback(this.failure("empty curr_htlc_temp_address_for_ht1a_pub_key"));
        }
        let msg = new pojo_1.Message();
    ...
    sendSignedHex100104(info, callback) {
        ...
        if (this.isNotString(info.curr_htlc_temp_address_for_he_pub_key)) {
            return callback(this.failure("empty curr_htlc_temp_address_for_he_pub_key"));
        }
        let msg = new pojo_1.Message();
    ...
    closeHTLCSigned(recipient_node_peer_id, recipient_user_peer_id, info, callback) {
        ...
        if (this.isNotString(info.last_htlc_temp_address_for_htnx_private_key)) {
            return callback(this.failure("empty last_htlc_temp_address_for_htnx_private_key"));
        }
        if (this.isNotString(info.curr_temp_address_pub_key)) {
            return callback(this.failure("empty curr_rsmc_temp_address_pub_key"));
        }
